...
class NeuralNetwork(object):
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        hidden_error = np.dot(output_error_term.item(), self.weights_hidden_to_output)
        print("hidden_error shape: {}".format(np.shape(hidden_error)))
        print(hidden_error)

        print("hidden_outputs shape: {}".format(np.shape(hidden_outputs)))

        hidden_error_term = hidden_error * hidden_outputs * (1 - hidden_outputs)
        print("hidden_error_term shape: {}".format(np.shape(hidden_error_term)))
    ...
    def run(self, features):
        ...
        final_outputs = self.activation_function(np.dot(final_inputs,self.weights_hidden_to_output)) # signals from final output layer
